...
describe('Search Menu', () => {
  context('Search Main', () => {
    ...
    it('contains - search modal with correct attrs', () => {
      cy.get(headerSelectors.searchMenu).find(modalSelectors.sectionId).should('have.length', 1);
      cy.get(headerSelectors.searchMenu).find(modalSelectors.sectionType).should('have.length', 1);
      cy.get(modalSelectors.headerContainer).should('have.class', VISUALLY_HIDDEN);
      /* eslint-disable promise/always-return, promise/catch-or-return, shopify/prefer-early-return*/
      cy.get(headerSelectors.searchMenu).find(modalSelectors.sectionId).then(($id) => {
