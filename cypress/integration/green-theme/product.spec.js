...
context('Product', () => {
  ...
  describe('CTA Submit - Cart Add AJAX', () => {
    ...
    it('Update Cart Quantity', () => {
      ...
      cy.get(productSelectors.submitButton).click();
      cy.wait('@getCartResponse').should((xhr) => {
        cy.get(cartSelectors.cartItemCount).should(
          'have.text',
          xhr.responseBody.quantity.toString(),
        );
      });
    });
    ...
    it('Product Card Popup closes properly, slide-down class replaced with slide-up - NO STUB', () => {
      ...
      cy.get(productCardSelector.modal)
        .find(modalSelectors.closeIcon)
        .click();
      cy.get(headerSelectors.productPopupContainer).should(
        'not.have.class',
    ...
    describe('Add To Cart Button functionality', () => {
      it('show loading while adding item to cart', () => {
        ...
        cy.wait(500);
        cy.get(productSelectors.submitButton).click();
        cy.get(productSelectors.submitButtonText).should(
          'have.class',
          VISUALLY_HIDDEN,
        );
      ...
      it('show error message when network down', () => {
        ...
        cy.get(productSelectors.submitFailure).should(
          'have.class',
          VISUALLY_HIDDEN,
        );
        cy.wait('@getCartResponse');
    ...
    describe('Populate correct data into product popup', () => {
      it('Product Popup show loading state before presenting data - NO STUB', () => {
        ...
        }).as('getAddCartResponse');
        // eslint-disable-next-line cypress/no-unnecessary-waiting
        cy.wait(500);
        cy.get(productSelectors.submitButton).click();
        cy.wait('@getAddCartResponse');
        cy.get(headerSelectors.productPopupContainer)
          .find(productCardSelector.quantity)
          .should('have.text', '1');
        cy.get(headerSelectors.productPopupContainer)
          .find(productCardSelector.title)
          .should(
            'have.text',
            '2018 Autumn Women Hoodie Casual Long Sleeve Hooded Pullover Sweatshirts Hooded Female Jumper Women Tracksuits Sportswear Clothes - Pink / S',
          );
        cy.get(headerSelectors.productPopupContainer)
          .find(productCardSelector.image)
          .find('img')
          .then((imgElement) => {
            expect(imgElement[0].alt).to.equal(
              '2018 Autumn Women Hoodie Casual Long Sleeve Hooded Pullover Sweatshirts Hooded Female Jumper Women Tracksuits Sportswear Clothes',
            );
            expect(imgElement[0].src).to.equal(
              'https://cdn.shopify.com/s/files/1/0258/8436/0792/products/product-image-851304014_300x.jpg?v=1569178286',
            );
          });
        cy.get(headerSelectors.productPopupContainer)
          .find(productCardSelector.price)
      ...
      it('Change product variant before adding to cart - NO STUB', () => {
        ...
        cy.get(headerSelectors.productPopupContainer)
          ...
          .should(
            'have.text',
            '2018 Autumn Women Hoodie Casual Long Sleeve Hooded Pullover Sweatshirts Hooded Female Jumper Women Tracksuits Sportswear Clothes - Black / M',
          );
        cy.get(headerSelectors.productPopupContainer)
          .find(productCardSelector.image)
          .find('img')
          .then((imgElement) => {
            ...
            expect(imgElement[0].dataset.src).to.equal(
              '//cdn.shopify.com/s/files/1/0258/8436/0792/products/product-image-851304013.jpg?v=1569178289',
            );
          });
        cy.get(headerSelectors.productPopupContainer)
          .find(productCardSelector.price)
